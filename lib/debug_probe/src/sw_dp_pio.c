...
void SWJ_Sequence(uint32_t count, const uint8_t* data) {
    ...
    PROBE_DEBUG("SWJ sequence count = %d FDB=0x%2x", count, data[0]);
    n = count;
    while(n > 0) {
        if(n > 8)
            bits = 8;
...
uint8_t SWD_Transfer(uint32_t request, uint32_t* data) {
    ...
    if(ack == DAP_TRANSFER_OK) {
        ...
        if(request & DAP_TRANSFER_RnW) {
            ...
            if(data) *data = val;
            PROBE_DEBUG("Read %02x ack %02x 0x%08x parity %01x", prq, ack, val, bit);
            ...
            PROBE_DEBUG("write %02x ack %02x 0x%08x parity %01x", prq, ack, val, parity);
        }
        /* Capture Timestamp */
